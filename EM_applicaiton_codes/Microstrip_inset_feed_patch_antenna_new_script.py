...
## It generates a rectangular micrsotrip patch antenna
## Simulates the patch antenna and generates results
...
FDTD = openEMS(NrTS=60000, CoordSystem=0, EndCriteria=1e-6) # Object FDTD is created/instantiated for the class openEMS, set number of time steps to 30000 and set energy decay criteria to be 0.000001
...
FDTD.SetBoundaryCond(['MUR', 'MUR', 'MUR', 'MUR', 'MUR', 'MUR']) # I have set it to MUR to compare with QucsStudio, it can also be set to PML_8 for more accuracy. Remember PML_8 will significantly increase simulaiton time!
...
####################
## Generate the microstrip patch geometry with PEC as material assignment
## Block 6 begins
...
start = [-((feed_line_width/2) + notch_width), -patch_length/2, substrate_thickness]
stop  = [((feed_line_width/2) + notch_width), -((patch_length/2) - cut_out_length), substrate_thickness]
...
start = [-feed_line_width/2, -((patch_length/2) + feed_line_length - cut_out_length), substrate_thickness]
stop  = [ feed_line_width/2 , -(patch_length/2 - cut_out_length), substrate_thickness]
...
### Microstrip patch geometry generated. Block 6 ends
####################
...
####################
## Generate simulaiton port plane
## Block 8 begins

# apply the excitation & resist as a current source
start = [0, -((patch_length/2) + feed_line_length - cut_out_length), 0]
stop  = [0, -((patch_length/2) + feed_line_length - cut_out_length), substrate_thickness]
port = FDTD.AddLumpedPort(1, feed_R, start, stop, 'z', 1.0, priority=14, edges2grid='xy') # A 2 dimension lumped port with voltage excitation is created and assigned the given priority 
...
CSX.Write2XML('Microstrip_patch.xml') # Geometry, problem setup gets dumped in .xml file, which can then be imported to AppCSXCADand verified
...
Power_in = 0.5*np.real(port.uf_tot * np.conj(port.if_tot)) # Antenna feed power, this is also called the accepted power. It is not the incident power, incident power is accepted power + reflected power. For a well matched antenna they are about the same at resonant frequency
#print(Power_in) # Debug step
...
efficiency = (power_radiated/Power_in[centre_index])*100 # It is how efficient the antenna radiates. Efficiency*Directivity = Gain dBi
...
print( 'Input power: Pin = {} Watt'.format(Power_in[centre_index]))# Prints input power
